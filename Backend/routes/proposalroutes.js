...
router.post("/upload", async (req, res) => {
  try {
    const { projectId, client, title, attachedFile } = req.body || {};
    ...
    const fileMeta = {
      name: attachedFile.name || "proposal",
    ...
    if (projectId) {
      try {
        ...
        await Project.findOneAndUpdate(q, { $set: { proposalPdf: fileMeta } });
      } catch (projErr) {
        console.error("Failed to attach proposal PDF to project:", projErr.message);
      }
    }
    res.json({ ...newProposal.toObject(), attachedFile: fileMeta });
  } catch (err) {
