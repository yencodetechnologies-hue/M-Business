...
:root {
  --primary: var(--app-accent, var(--app-accent, #2563EB)); --primary-dark:#2563EB; --primary-light:var(--teal-light, #EFF6FF); --primary-mid:#E2E8F0;
...
.aud-topnav{background:#1E293B;border-bottom:1px solid #1E293B;padding:0 32px;height:62px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100;}
.aud-brand{display:flex;align-items:center;gap:10px;}
.aud-brand-icon{width:36px;height:36px;background:linear-gradient(135deg,#2563EB,#2563EB);border-radius:10px;display:flex;align-items:center;justify-content:center;}
.aud-brand-icon i{font-size:18px;color:#FFFFFF;}
.aud-brand-name{font-size:16px;font-weight:900;color:#FFFFFF;}
...
.aud-hero{background:linear-gradient(135deg,#1E293B,#1E293B);border-radius:var(--radius);padding:24px 28px;margin-bottom:22px;display:flex;align-items:center;justify-content:space-between;}
.aud-hero-left h1{font-size:19px;font-weight:900;color:#FFFFFF;margin-bottom:4px;}
...
.ahs .n{font-size:20px;font-weight:900;color:#FFFFFF;}
...
.atab.active{color:var(--app-accent);border-bottom-color:var(--app-accent);}
.atab:hover:not(.active){background:var(--bg);}
.tab-notif{background:var(--app-text);color:#FFFFFF;font-size:10px;font-weight:800;border-radius:20px;padding:1px 6px;margin-left:4px;}
...
.flag-btn{background:var(--app-accent-light);color:var(--app-muted);border:1px solid var(--app-muted);border-radius:8px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;}
.note-btn{background:var(--app-accent-light);color:var(--app-accent);border:1px solid var(--app-accent);border-radius:8px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;}
.flagged-row td{background:#F8FAFC !important;}
.audit-note{font-size:11px;color:var(--app-muted);font-style:italic;margin-top:3px;display:flex;align-items:center;gap:4px;}
.grid-2{display:grid;grid-template-columns:1fr 1fr;gap:20px;}

.av{border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;color:#FFFFFF;flex-shrink:0;}
        ...
        <div className="aud-topnav">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div className="aud-chip">
              <div className="av av-sm" style={{ background: '#2563EB' }}>RA</div>Rajan Auditors & Co.
            </div>
        ...
        <div className="aud-page">
          ...
          {activeTab === 'statements' && (
            <div>
              ...
              <div className="dl-card">
                <div className="dl-icon" style={{ background: 'var(--app-accent-light)' }}><i className="ti ti-arrow-bar-up" style={{ color: 'var(--app-text)' }}></i></div>
                    ...
                    {loading ? null : transactions.length === 0 ? <tr><td colSpan="7">No transactions found.</td></tr> : transactions.map((txn, i) => {
                      ...
                      return (
                        <tr key={i} className={isFlagged ? "flagged-row" : ""}>
                          ...
                          <td style={{ color: isFlagged ? 'var(--app-muted)' : 'var(--primary)', fontWeight: 700 }}>{id}</td>
                          ...
                          <td><div style={{ display: 'flex', gap: '5px' }}><button className="flag-btn" style={isFlagged ? { background: 'var(--app-muted)', color: '#FFFFFF' } : {}} onClick={() => flagRow(txn)}><i className="ti ti-flag"></i>{isFlagged ? 'Flagged' : 'Flag'}</button><button className="note-btn" onClick={() => addNoteQuick(txn)}><i className="ti ti-message-2"></i>Note</button></div></td>
                        </tr>
                      );
                    })}
                  </tbody>
          ...
          {activeTab === 'notes' && (
            <div>
              <div className="card" style={{ marginBottom: '18px' }}>
                <div style={{ fontSize: '14px', fontWeight: 800, marginBottom: '16px' }}><i className="ti ti-message-2" style={{ color: 'var(--app-accent)' }}></i>Add Audit Note</div>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  <button className="btn" style={{ background: 'var(--app-accent-light)', color: 'var(--app-muted)', border: '1px solid var(--app-muted)' }} onClick={() => saveNote('flag')}><i className="ti ti-flag"></i>Flag Transaction</button>
                  <button className="btn" style={{ background: 'var(--app-accent-light)', color: 'var(--app-accent)', border: '1px solid var(--app-accent)' }} onClick={() => saveNote('note')}><i className="ti ti-message-2"></i>Save Note</button>
                </div>
              </div>
              <div className="card">
                ...
                {loading ? <div>Loading notes...</div> : notes.length === 0 ? <div>No audit notes added yet.</div> : notes.map((n, i) => (
                  <div key={i} style={{ padding: '14px', background: n.flagged ? 'var(--app-accent-light)' : 'var(--app-accent-light)', borderRadius: '10px', borderLeft: `4px solid ${n.flagged ? 'var(--app-muted)' : 'var(--app-accent)'}`, marginBottom: '12px' }}>
                    <div style={{ fontSize: '12px', fontWeight: 800, color: n.flagged ? 'var(--app-muted)' : 'var(--app-accent)', marginBottom: '4px' }}><i className={n.flagged ? "ti ti-flag" : "ti ti-message-2"}></i>{n.flagged ? 'FLAGGED' : 'NOTE'} — {n.transactionId} ({new Date(n.createdAt).toLocaleDateString()})</div>
