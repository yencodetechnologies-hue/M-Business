...
export default function AuthPage({ setUser, initialTab = "login" }) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // Pre-fill the login form's email so the user only has to type their
      // password, then send them to the Login tab. No auto-login — they must
      // authenticate themselves before reaching the Dashboard.
      if (payload.email) {
        localStorage.setItem(`justRegistered:${payload.email}`, "1");
      }
      setLoginData(p => ({ ...p, email: payload.email }));
