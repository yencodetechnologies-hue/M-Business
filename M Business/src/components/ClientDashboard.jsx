              ...
              <div className="prop-doc" style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #e0eef0", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", maxHeight: "none", overflow: "visible" }}
                dangerouslySetInnerHTML={{
                  __html: (() => {
                    ...
                    div.innerHTML = prop.html || "";
                    div.querySelectorAll(".sop").forEach(el => {
                      const wrapper = el.closest(".ps") || el.parentElement;
...
export default function ClientDashboard({ user: userProp, setUser, portalMode = false, themeColor = "#00BCD4" }) {
  ...
  const [reviewQuotation, setReviewQuotation] = useState(null);

  const handleQuotationApprove = async (id) => {
    setQuotations(prev => prev.map(q => q.id === id ? { ...q, status: "approved" } : q));
    try {
      await axios.patch(`${BASE_URL}/api/quotations/${id}/status`, { status: "approved" });
    } catch (err) {
      console.error("Failed to approve quotation", err);
    }
  };

  const handleQuotationReviewSubmit = async (id, comment) => {
    setQuotations(prev => prev.map(q => q.id === id ? { ...q, status: "reviewed", clientComment: comment } : q));
    try {
      await axios.patch(`${BASE_URL}/api/quotations/${id}/status`, { status: "reviewed", comment });
    } catch (err) {
