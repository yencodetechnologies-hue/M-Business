    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 20, padding: "12px 0 4px", borderTop: "1px solid var(--app-border)", flexWrap: "wrap", gap: 15 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <select 
          ...
          style={{ padding: "7px 12px", borderRadius: 10, border: "1.5px solid var(--app-border)", fontSize: 13, background: "var(--app-bg)", color: "#1e0a3c", outline: "none", cursor: "pointer", fontWeight: 500 }}
        >
...
function ActionBtns({onView,onEdit,onDelete}){
  return(
    <div style={{display:"flex",gap:5,flexWrap:"nowrap"}}>
      ...
      <button onClick={onEdit} title="Edit" style={{background:"rgba(147,51,234,0.1)",border:"1px solid rgba(147,51,234,0.3)",borderRadius:7,padding:"5px 10px",fontSize:12,color:"var(--app-accent)",cursor:"pointer",fontWeight:600,fontFamily:"inherit",whiteSpace:"nowrap"}}>✏️ Edit</button>
...
function InfoRow({icon,label,value}){
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:"var(--app-bg)",borderRadius:9,border:"1px solid var(--app-border)",marginBottom:7}}>
      ...
      <div><div style={{fontSize:10,color:"var(--app-accent)",fontWeight:700,letterSpacing:0.5,textTransform:"uppercase"}}>{label}</div><div style={{fontSize:13,fontWeight:600,color:"#1e0a3c",marginTop:1}}>{value}</div></div>
    </div>
  );
}
        ...
        <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,background:"#fff",border:"1.5px solid var(--app-border)",borderRadius:12,boxShadow:"0 8px 32px rgba(147,51,234,0.15)",zIndex:999,overflow:"hidden"}}>
          ...
          <div style={{maxHeight:180,overflowY:"auto"}}>
            {filtered.length===0?<div style={{padding:14,textAlign:"center",color:"#a78bfa",fontSize:13}}>No clients found</div>
              :filtered.map((c,i)=>{const name=c.clientName||c.name||"";const company=c.companyName||c.company||"";const isSel=value===name;return(<div key={i} onClick={()=>{onChange(name);setOpen(false);setSearch("");}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",cursor:"pointer",background:isSel?"var(--app-border)":"transparent",borderBottom:"1px solid var(--app-bg)"}} onMouseEnter={e=>e.currentTarget.style.background="var(--app-bg)"} onMouseLeave={e=>e.currentTarget.style.background=isSel?"var(--app-border)":"transparent"}><div style={{width:28,height:28,borderRadius:"50%",background:c.logoUrl?"#fff":"linear-gradient(135deg,var(--app-accent),var(--app-accent))",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:11,fontWeight:700,flexShrink:0,overflow:"hidden",border:c.logoUrl?"1px solid var(--app-border)":"none"}}>{c.logoUrl?<img src={c.logoUrl} alt="logo" style={{width:"100%",height:"100%",objectFit:"contain"}}/>:(name[0]?.toUpperCase()||"?")}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:T.text}}>{name}</div>{company&&<div style={{fontSize:11,color:"#a78bfa"}}>{company}</div>}</div>{isSel&&<span style={{fontSize:14,color:"var(--app-accent)"}}>✓</span>}</div>);})}
          </div>
        </div>
      )}
                  ...
                  <tr key={c._id||i} style={{borderBottom:"1px solid #f3f0ff"}} onMouseEnter={e=>e.currentTarget.style.background="var(--app-bg)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                    ...
                    <td style={{padding:"12px 14px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <div style={{width:28,height:28,borderRadius:"50%",background:c.logoUrl?"#fff":"linear-gradient(135deg,var(--app-accent),var(--app-accent))",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:11,fontWeight:700,flexShrink:0,overflow:"hidden",border:c.logoUrl?"1px solid var(--app-border)":"none"}}>{c.logoUrl?<img src={c.logoUrl} alt="logo" style={{width:"100%",height:"100%",objectFit:"contain"}}/>:((c.clientName||c.name||"?")[0].toUpperCase())}</div>
      ...
      {viewClient&&(
        <Mdl title="Client Profile" onClose={()=>setViewClient(null)} maxWidth={500}>
          ...
          <div style={{display:"flex",gap:10,marginTop:16}}>
            <button onClick={()=>{setViewClient(null);openEdit(viewClient);}} style={{flex:1,padding:"10px",background:"linear-gradient(135deg,var(--app-accent),var(--app-accent))",border:"none",borderRadius:10,fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>✏️ Edit</button>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              {paginated.length===0?<tr><td colSpan={9} style={{padding:30,textAlign:"center",color:"#a78bfa"}}>No employees found</td></tr>
                :paginated.map((e,i)=>(
                  <tr key={e._id||i} style={{borderBottom:"1px solid #f3f0ff"}} onMouseEnter={ev=>ev.currentTarget.style.background="var(--app-bg)"} onMouseLeave={ev=>ev.currentTarget.style.background="transparent"}>
                    ...
                    <td style={{padding:"12px 14px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <div style={{width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,var(--app-accent),var(--app-accent))",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:11,fontWeight:700,flexShrink:0}}>{(e.name||"?")[0].toUpperCase()}</div>
      ...
      {viewEmp&&(
        <Mdl title="Employee Profile" onClose={()=>setViewEmp(null)} maxWidth={500}>
          <div style={{display:"flex",alignItems:"center",gap:14,padding:16,background:"linear-gradient(135deg,var(--app-bg),var(--app-bg))",borderRadius:14,border:"1px solid var(--app-border)",marginBottom:18}}>
            <div style={{width:52,height:52,borderRadius:"50%",background:"linear-gradient(135deg,var(--app-accent),var(--app-accent))",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:20,fontWeight:800,flexShrink:0}}>{(viewEmp.name||"?")[0].toUpperCase()}</div>
            <div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,minWidth:800}}>
            ...
            <tbody>
              {paginated.length===0?<tr><td colSpan={7} style={{padding:30,textAlign:"center",color:"#a78bfa"}}>No projects found</td></tr>
                :paginated.map((p,i)=>(
                  <tr key={p._id||i} style={{borderBottom:"1px solid #f3f0ff"}} onMouseEnter={ev=>ev.currentTarget.style.background="var(--app-bg)"} onMouseLeave={ev=>ev.currentTarget.style.background="transparent"}>
      ...
      {viewProj&&(
        <Mdl title="Project Details" onClose={()=>setViewProj(null)} maxWidth={550}>
          ...
          <div style={{display:"flex",gap:10,marginTop:16}}>
            <button onClick={()=>{setViewProj(null);openEdit(viewProj);}} style={{flex:1,padding:"10px",background:"linear-gradient(135deg,var(--app-accent),var(--app-accent))",border:"none",borderRadius:10,fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>✏️ Edit</button>
...
function SearchDropdown({label,items,displayKey,value,onChange,error,placeholder}){
  ...
  return(
    <div style={{marginBottom:14,position:"relative"}}>
      <label style={{display:"block",fontSize:11,color:"var(--app-accent)",fontWeight:700,letterSpacing:0.5,marginBottom:5}}>{label.toUpperCase()}</label>
      <div onClick={()=>setOpen(!open)} style={{width:"100%",border:`1.5px solid ${error?"#EF4444":open?"var(--app-accent)":"var(--app-border)"}`,borderRadius:10,padding:"10px 36px 10px 14px",fontSize:13,color:value?T.text:"#a78bfa",background:"var(--app-bg)",cursor:"pointer",position:"relative",userSelect:"none",minHeight:42,boxSizing:"border-box"}}>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div className="dash-stats" style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12}}>
        {tsStats.map(({t,v,i,c})=>(<div key={t} style={{background:"#fff",borderRadius:14,padding:"16px 14px",boxShadow:"0 4px 18px rgba(147,51,234,0.07)",border:"1px solid var(--app-border)",position:"relative",overflow:"hidden"}}><div style={{width:38,height:38,borderRadius:10,background:`${c}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,marginBottom:8}}>{i}</div><div style={{fontSize:10,color:"#a78bfa",fontWeight:700,letterSpacing:0.5,marginBottom:2}}>{t.toUpperCase()}</div><div style={{fontSize:24,fontWeight:800,color:c}}>{v}</div></div>))}
      </div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          <button onClick={()=>setTsFilter("All")} style={{padding:"7px 13px",borderRadius:9,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",border:"1.5px solid",borderColor:tsFilter==="All"?"var(--app-accent)":"var(--app-border)",background:tsFilter==="All"?"rgba(147,51,234,0.1)":"#fff",color:tsFilter==="All"?"var(--app-accent)":"#a78bfa"}}>All</button>
          {customStatuses.map(f=>(<button key={f} onClick={()=>setTsFilter(f)} style={{padding:"7px 13px",borderRadius:9,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",border:"1.5px solid",borderColor:tsFilter===f?"var(--app-accent)":"var(--app-border)",background:tsFilter===f?"rgba(147,51,234,0.1)":"#fff",color:tsFilter===f?"var(--app-accent)":"#a78bfa"}}>{f}</button>))}
        </div>
      ...
      {tsModal&&(<Mdl title={tsModal==="add"?"Add Project Status":"Edit Project Status"} onClose={()=>setTsModal(null)}>
        <div className="modal-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 18px"}}>
          ...
          <div style={{marginBottom:14}}><label style={{display:"block",fontSize:11,color:"var(--app-accent)",fontWeight:700,letterSpacing:0.5,marginBottom:5}}>CLIENT *</label><ClientDropdown clients={clientNames.length?clients:[]} value={tsForm.client} onChange={v=>{setTsForm({...tsForm,client:v});setTsErr(p=>({...p,client:""}));}} error={tsErr.client}/>{tsErr.client&&<div style={{fontSize:11,color:"#EF4444",marginTop:4}}>⚠️ {tsErr.client}</div>}</div>
...
const finalResumeUrl=resumeUrl;return(
                  <tr key={c._id||c.id||i} style={{borderBottom:"1px solid #f3f0ff",transition:"background 0.15s"}} onMouseEnter={e=>e.currentTarget.style.background="var(--app-bg)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                    ...
                    <td style={{padding:"12px 12px"}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:30,height:30,borderRadius:"50%",background:"linear-gradient(135deg,var(--app-accent),var(--app-accent))",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:11,fontWeight:700,flexShrink:0}}>{(c.name||"?")[0].toUpperCase()}</div><span style={{fontWeight:700,color:"#1e0a3c"}}>{c.name||"—"}</span></div></td>
                    <td style={{padding:"12px 12px"}}><div style={{fontSize:12,color:"var(--app-accent)"}}>{c.email||"—"}</div><div style={{fontSize:11,color:"#a78bfa",marginTop:2}}>{c.mobile||""}</div></td>
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(59,7,100,0.6)",backdropFilter:"blur(10px)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onClose}>
      <div style={{background:"#fff",borderRadius:22,width:"100%",maxWidth:420,maxHeight:"90vh",boxShadow:"0 32px 80px rgba(147,51,234,0.3)",display:"flex",flexDirection:"column",overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
        <div style={{background:"linear-gradient(135deg,var(--app-accent),var(--app-accent),var(--app-accent))",padding:"28px 28px 22px",textAlign:"center",flexShrink:0}}>
        ...
        <div style={{padding:"12px 24px 18px",borderTop:"1px solid var(--app-border)",flexShrink:0}}>
          <div style={{display:"flex",gap:10}}>
            <button onClick={onClose} style={{flex:1,padding:"10px",background:"var(--app-bg)",border:"1px solid var(--app-border)",borderRadius:9,fontSize:13,fontWeight:600,color:"#1e0a3c",cursor:"pointer",fontFamily:"inherit"}}>Close</button>
            <button onClick={()=>logoRef.current.click()} style={{flex:1,padding:"10px",background:"linear-gradient(135deg,var(--app-accent),var(--app-accent))",border:"none",borderRadius:9,fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>📷 Upload Logo</button>
...
export default function Dashboard({setUser,user,fixedLogo}){
    ...
    <div style={{display:"flex",minHeight:"100vh",background:"#f8fafc",fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      ...
      <div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column"}}>
        <div className="mob-topbar" style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",background:"#fff",borderBottom:"1px solid var(--app-border)",position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 8px rgba(147,51,234,0.07)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:30,height:30,background:"linear-gradient(135deg,var(--app-accent),var(--app-accent))",borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:14,color:"#fff"}}>
              {companyNameStr[0] || "W"}
            </div>
        ...
        <div className="main-content" style={{flex:1,padding:"22px 24px",overflowY:"auto"}}>
          <div className="page-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
            ...
            <div className="header-actions" style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
              ...
              {validActive==="projects"&&<button onClick={()=>{setNpError({});setModal("project");}} style={B("var(--app-accent)")}>+ New Project</button>}
          ...
          {validActive==="dashboard"&&<>
            <div className="dash-stats" style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12,marginBottom:18}}>
              {[{t:"Total Clients",v:clients.length,i:"👥",c:"var(--app-accent)"},{t:"Employees",v:employees.length,i:"👨‍💼",c:"var(--app-accent)"},{t:"Managers",v:managers.length,i:"🧑‍💼",c:"#f59e0b"},{t:"Projects",v:projects.length,i:"📁",c:"var(--app-accent)"},{t:"Invoices",v:INVOICES.length,i:"🧾",c:"#22C55E"}].map(({t,v,i,c})=>(
                <div key={t} style={{background:"#fff",borderRadius:14,padding:"16px 14px",boxShadow:"0 4px 18px rgba(147,51,234,0.07)",border:"1px solid var(--app-border)",position:"relative",overflow:"hidden"}}>
            ...
            <div className="dash-2col" style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:14,marginBottom:14}}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 300 }}>
                    <thead>
                      <tr style={{ background: "var(--app-bg)" }}>
                        {["Project", "Client", "Status", "View"].map(c => <th key={c} style={{ padding: "8px 10px", textAlign: "left", color: "#a78bfa", fontWeight: 700, fontSize: 11, borderBottom: "2px solid var(--app-border)" }}>{c.toUpperCase()}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {projects.slice(0, 5).map((p, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid var(--app-bg)" }}>
                          ...
                          <td style={{ padding: "9px 10px" }}>
                            <button onClick={() => setViewProject(p)} style={{ background: "linear-gradient(135deg,var(--app-accent),var(--app-accent))", border: "none", borderRadius: 6, padding: "5px 12px", fontSize: 11, fontWeight: 700, color: "#fff", cursor: "pointer", fontFamily: "inherit" }}>View</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <SC title="Recent Activity">{[{icon:"👤",text:"New client added",time:"2m ago",c:"var(--app-accent)"},{icon:"👨‍💼",text:"Employee joined",time:"30m ago",c:"var(--app-accent)"},{icon:"🧾",text:"Invoice created",time:"1h ago",c:"#22C55E"},{icon:"📁",text:"Project updated",time:"3h ago",c:"var(--app-accent)"},{icon:"✅",text:"ERP completed",time:"2d ago",c:"#F59E0B"}].map((a,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",borderBottom:i<4?"1px solid var(--app-bg)":"none"}}><div style={{width:28,height:28,borderRadius:8,background:`${a.c}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,flexShrink:0}}>{a.icon}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:600,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.text}</div><div style={{fontSize:11,color:"#a78bfa"}}>{a.time}</div></div></div>))}</SC>
            </div>
            <div className="dash-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              <SC title="Project Progress">{TRACKING_SEED.map(t=>(<div key={t.id} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:13,fontWeight:600,color:T.text}}>{t.name}</span><span style={{fontSize:12,fontWeight:700,color:sc(t.status)}}>{t.pct}%</span></div><div style={{background:"var(--app-border)",borderRadius:6,height:6}}><div style={{width:`${t.pct}%`,background:t.pct===100?"linear-gradient(90deg,#22C55E,#4ade80)":"linear-gradient(90deg,var(--app-accent),var(--app-accent))",borderRadius:6,height:"100%"}}/></div><div style={{fontSize:11,color:"#a78bfa",marginTop:2}}>{t.client}</div></div>))}</SC>
              <SC title="Invoice Status">{INVOICES.map(inv=>(<div key={inv.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid var(--app-bg)"}}><div><div style={{fontSize:13,fontWeight:600,color:T.text}}>{inv.id} · {inv.client}</div><div style={{fontSize:11,color:"#a78bfa"}}>Due: {inv.due}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:13,fontWeight:700,color:T.text,marginBottom:3}}>{inv.total}</div><Badge label={inv.status}/></div></div>))}</SC>
            </div>
          </>}
      ...
      {modal==="project"&&<Mdl title="Create New Project" onClose={()=>setModal(null)}>
        <div className="modal-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 18px"}}>
          ...
          <div style={{marginBottom:14}}>
            <label style={{display:"block",fontSize:11,color:"var(--app-accent)",fontWeight:700,letterSpacing:0.5,marginBottom:5}}>CLIENT NAME *</label>
        ...
        <div style={{marginBottom:14}}>
          <label style={{display:"block",fontSize:11,color:"var(--app-accent)",fontWeight:700,letterSpacing:0.5,marginBottom:5}}>ASSIGN EMPLOYEES <span style={{fontSize:10,color:"#a78bfa",fontWeight:400}}>(select multiple)</span></label>
          <div style={{border:"1.5px solid var(--app-border)",borderRadius:10,padding:"12px",background:"var(--app-bg)",maxHeight:200,overflowY:"auto"}}>
            {employees.length===0?<div style={{color:"#a78bfa",fontSize:13,textAlign:"center",padding:"20px"}}>No employees available</div>
              :employees.map(e=>(
  ...
  if(evt.target.checked){
    ...
    setNp(prev=>({...prev,assignedTo:prev.assignedTo.filter(n=>n!==e.name)}));
        ...
        <div style={{display:"flex",justifyContent:"flex-end",gap:10,marginTop:6}}>
          <button onClick={()=>setModal(null)} style={{background:"var(--app-bg)",border:"1px solid var(--app-border)",color:T.text,borderRadius:10,padding:"10px 16px",cursor:"pointer",fontWeight:600,fontSize:13}}>Cancel</button>
          <button onClick={addProject} disabled={projSaveLoading} style={{...B("var(--app-accent)"),opacity:projSaveLoading?0.7:1}}>{projSaveLoading?"Saving...":"Save Project →"}</button>
        </div>
      </Mdl>}
      ...
      {viewProject&&(
        <Mdl title="Project Details" onClose={()=>setViewProject(null)} maxWidth={550}>
          ...
          <div style={{display:"flex",gap:10,marginTop:16}}>
            <button onClick={()=>setViewProject(null)} style={{flex:1,padding:"10px",background:"linear-gradient(135deg,var(--app-accent),var(--app-accent))",border:"none",borderRadius:10,fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>Close</button>
          </div>
        </Mdl>
      )}
    </div>
