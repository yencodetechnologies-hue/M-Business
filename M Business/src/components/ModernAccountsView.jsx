    ...
    <div style={{ paddingBottom: 60, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 32 }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          ...
          <button style={{ padding: "10px 16px", background: "linear-gradient(135deg, var(--app-accent), var(--app-accent2, var(--app-accent)))", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 13, color: "#FFFFFF", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, boxShadow: "0 4px 12px rgba(var(--app-accent-rgb, 99, 102, 241), 0.3)" }}>
             Statement
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24, marginBottom: 32 }}>
        {/* PRIMARY */}
        <div style={{ background: "linear-gradient(135deg, #1E293B, #0F172A)", borderRadius: 24, padding: 24, color: "#FFFFFF", position: "relative", overflow: "hidden", boxShadow: "0 10px 30px rgba(15, 23, 42, 0.2)" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, var(--app-accent), var(--app-accent2, var(--app-accent)))", borderRadius: 24, padding: 24, color: "#FFFFFF", position: "relative", overflow: "hidden", boxShadow: "0 10px 30px rgba(var(--app-accent-rgb, 99, 102, 241), 0.25)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 30, position: "relative", zIndex: 1 }}>
        ...
        <div style={{ background: "var(--app-card)", border: "1px solid var(--app-border)", borderRadius: 24, padding: 24, position: "relative", overflow: "hidden", boxShadow: "0 10px 30px rgba(0,0,0,0.03)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 30 }}>
            <div style={{ background: "rgba(22, 163, 74, 0.1)", color: "#16A34A", padding: "6px 12px", borderRadius: 20, fontSize: 12, fontWeight: 800, letterSpacing: 0.5 }}>Net Balance</div>
          </div>
          <div>
            ...
            {[...income.map(i => ({ ...i, type: "income" })), ...expenses.map(e => ({ ...e, type: "expense" }))]
              ...
              .map((item, idx) => (
                <div key={idx} style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px", borderRadius: 16, background: "var(--app-bg)", border: "1px solid var(--app-border)", transition: "transform 0.2s" }} onMouseEnter={e => e.currentTarget.style.transform="translateX(4px)"} onMouseLeave={e => e.currentTarget.style.transform="translateX(0)"}>
                  <div style={{ width: 48, height: 48, borderRadius: 14, background: item.type === "income" ? "rgba(22, 163, 74, 0.1)" : "rgba(100, 116, 139, 0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, color: item.type === "income" ? "#16A34A" : "#64748B" }}>
                    {item.type === "income" ? "" : ""}
                  </div>
                  <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 900, color: item.type === "income" ? "#16A34A" : "var(--app-text)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          <div style={{ background: "var(--app-card)", border: "1px solid var(--app-border)", borderRadius: 24, padding: 24, boxShadow: "0 10px 30px rgba(0,0,0,0.02)" }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 24, position: "relative" }}>
              <div style={{ width: 160, height: 160, borderRadius: "50%", background: denom > 0 ? `conic-gradient(var(--app-accent) 0% ${incomePct}%, #64748B ${incomePct}% 100%)` : "var(--app-border)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13, fontWeight: 600 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ width: 10, height: 10, borderRadius: "50%", background: "#64748B" }}></div><span style={{ color: "var(--app-muted)" }}>Expenses</span></div>
                <div style={{ color: "var(--app-text)", fontWeight: 800 }}>{expensePct}%</div>
              </div>
            </div>
          </div>

          <div style={{ background: "linear-gradient(135deg, var(--app-accent), var(--app-accent2, var(--app-accent)))", borderRadius: 24, padding: 24, color: "#FFFFFF", boxShadow: "0 10px 30px rgba(var(--app-accent-rgb, 99, 102, 241), 0.25)" }}>
            ...
            <button style={{ background: "#FFFFFF", color: "var(--app-accent)", border: "none", padding: "10px 20px", borderRadius: 12, fontWeight: 800, fontSize: 13, cursor: "pointer", width: "100%" }}>Check Eligibility</button>
          </div>
        </div>
