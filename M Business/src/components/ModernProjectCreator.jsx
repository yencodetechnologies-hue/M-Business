...
export default function ModernProjectCreator({ onBack, clients = [], employees = [], onSuccess, editProject, prefillClient, onAddEmployeeClick }) {
  ...
  const [newUpdateTeamMembers, setNewUpdateTeamMembers] = useState([]);
  const [showUpdateTeamDropdown, setShowUpdateTeamDropdown] = useState(false);
  const [newUpdateType, setNewUpdateType] = useState('');
  const [newUpdateIsApproval, setNewUpdateIsApproval] = useState(false);
  const [newUpdateTitle, setNewUpdateTitle] = useState('');
  const [newUpdateText, setNewUpdateText] = useState('');
  const [postingNewUpdate, setPostingNewUpdate] = useState(false);
  ...
  const postNewUpdate = async () => {
    if (!newUpdateTitle.trim()) { alert('Update title is required.'); return; }
    setPostingNewUpdate(true);
    try {
      const newUpdate = {
        ...
        author: 'Admin',
        type: newUpdateType || 'general',
        recipients: newUpdateTeamMembers,
        isApprovalRequest: newUpdateIsApproval,
        approvalStatus: newUpdateIsApproval ? 'pending' : undefined,
        status: 'sent',
      };
      const newUpdates = [newUpdate, ...projectUpdates];
      await persistUpdates(newUpdates);
      setProjectUpdates(newUpdates);
      setNewUpdateTitle('');
      setNewUpdateText('');
      setNewUpdateType('');
      setNewUpdateTeamMembers([]);
      setNewUpdateIsApproval(false);
    } catch (err) {
      console.error('Failed to post update:', err);
      alert('Failed to post update.');
    } finally {
      setPostingNewUpdate(false);
    }
  };
  ...
  const handleDeleteProject = async () => {
    if (!editProject?._id && !editProject?.id) return;
    ...
    try {
      ...
      if (onSuccess) {
        await onSuccess({ _deleted: true, _id: projectId });
      } else if (onBack) {
        onBack();
      }
    } catch (err) {
                    ...
                    {assignedEmps.map((emp, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span style={{ fontSize: 13, color: P.textDark, fontWeight: 600 }}>{empName}</span>
                          <button
                            type="button"
                ...
                >
                      ...
                      return (
                        <label
                          ...
                          style={{
                            ...
                            fontSize: 14, fontFamily: 'Nunito,sans-serif', color: P.textDark
                          }}
                        >
                          <input
                            type="checkbox"
                            checked={isChecked}
                            onChange={() => {
                              setSelectedEmpsToAdd(prev =>
                                isChecked ? prev.filter(n => n !== empName) : [...prev, empName]
                              );
                            }}
                          />
                          {empName}{emp.department ? ` (${emp.department})` : ''}
                        </label>
                      );
                    })}
                  ...
                  <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                    <button
                      ...
                      onClick={() => { setShowAddEmployee(false); setSelectedEmpsToAdd([]); }}
                    >
                      Cancel
                    </button>
                    <button
                      className="mpc-btn mpc-btn-primary"
                      onClick={() => {
                        if (selectedEmpsToAdd.length === 0) return;
                        setAssigned(prev => [...new Set([...prev, ...selectedEmpsToAdd])]);
                        setSelectedEmpsToAdd([]);
