...
export default function QuotationCreator({ clients = [], projects = [], companyLogo, onLogoChange, onConvertToInvoice }) {
  ...
  const handleConvert = async (entry) => {
    ...
    try {
      const res  = await fetch(`http://localhost:5000/api/quotations/${entry.id}/convert`, { method: "POST" });
      const data = await res.json();
      if (data.success) {
  ...
  const handleStatusChange = async (entry, newStatus) => {
    try {
      await fetch(`http://localhost:5000/api/quotations/${entry.id}/status`, {
