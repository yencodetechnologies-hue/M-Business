...
.ps-lbl{font-size:9px;font-weight:700;color: var(--app-accent, #00BCD4);text-transform:uppercase;letter-spacing:.8px;margin-bottom:8px;display:flex;align-items:center;gap:5px}
...
.pb-lbl{font-size:8px;font-weight:700;color: var(--app-accent, #00BCD4);text-transform:uppercase;letter-spacing:.7px;margin-bottom:3px}
...
.exec-block.solution{border-left-color: var(--app-accent, #00BCD4);background:var(--teal-lighter, #F0FDFE)}
...
.exec-block.solution .eb-lbl{color: var(--app-accent, #00BCD4)}
...
.del-item-p::before{content:'✓';color: var(--app-accent, #00BCD4);font-weight:800;font-size:11px;flex-shrink:0}
...
.tl-dot{width:20px;height:20px;border-radius:50%;background: var(--app-accent, #00BCD4);display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:800;color:#fff;flex-shrink:0}
.tl-line-p{width:2px;background:var(--teal-light, #E0F7FA);flex:1;margin:2px 0;min-height:10px}
.tl-pi:last-child .tl-line-p{display:none}
.tl-pi-title{font-size:10px;font-weight:700;color:#1A2E35}
.tl-pi-date{font-size:9px;color: var(--app-accent, #00BCD4);font-weight:600}
...
.pricing-tbl thead tr{background:linear-gradient(135deg, var(--app-accent, #00BCD4),#006E7F)}
...
.pricing-grand{display:flex;justify-content:space-between;padding:6px 8px;background:linear-gradient(135deg, var(--app-accent, #00BCD4),#006E7F);border-radius:6px;margin-top:5px}
...
.cs-p{padding:8px 10px;background:#F8FAFB;border-radius:8px;border-left:3px solid  var(--app-accent, #00BCD4);margin-bottom:7px}
...
.tm-p{padding:8px 10px;background:var(--teal-lighter, #F0FDFE);border-radius:8px;border:1px solid var(--teal-light, #E0F7FA);margin-bottom:7px;font-style:italic}
.tm-p:last-child{margin-bottom:0}
.tm-p-text{font-size:10px;color:#607D86;margin-bottom:5px}
.tm-p-author{font-size:9px;font-weight:700;color: var(--app-accent, #00BCD4)}
...
const HARDCODED_VARS = `
  ...
  --purple:#7C5CFC; --purple-bg:#EEE9FF;
  --blue:#2563EB; --blue-bg:#EFF4FF;
  --app-accent: var(--app-accent, #00BCD4);
  --font:'Nunito',sans-serif;
`;
...
const PRINT_BASE_CSS = `
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .ps-lbl { color: var(--app-accent, #00BCD4) !important; }
  .pb-lbl { color: var(--app-accent, #00BCD4) !important; }
  .exec-block.problem { border-left-color:#F05C5C !important; background:#FEF2F2 !important; }
  .exec-block.solution { border-left-color: var(--app-accent, #00BCD4) !important; background:var(--teal-lighter, #F0FDFE) !important; }
  ...
  .exec-block.solution .eb-lbl { color: var(--app-accent, #00BCD4) !important; }
  .exec-block.whyus .eb-lbl { color:#26C281 !important; }
  .tl-dot { background: var(--app-accent, #00BCD4) !important; }
  .tl-line-p { background:var(--teal-light, #E0F7FA) !important; }
  .tl-pi-date { color: var(--app-accent, #00BCD4) !important; }
  .del-item-p::before { color: var(--app-accent, #00BCD4) !important; content:'✓' !important; }
  .pricing-tbl thead tr { background:linear-gradient(135deg, var(--app-accent, #00BCD4),#006E7F) !important; }
  .pricing-grand { background:linear-gradient(135deg, var(--app-accent, #00BCD4),#006E7F) !important; }
  ...
  .tm-p { background:var(--teal-lighter, #F0FDFE) !important; border:1px solid var(--teal-light, #E0F7FA) !important; }
  .cs-p { border-left-color: var(--app-accent, #00BCD4) !important; background:#F8FAFB !important; }
...
function getTheme(name) {
  return THEMES.find(x => x.name === name) || { p: " var(--app-accent, #00BCD4)", g: "linear-gradient(135deg, var(--app-accent, #00BCD4),#006E7F)" };
}
...
function getElementsHTML(elements) {
  ...
  return `<div style="position:absolute;inset:0;pointer-events:none;z-index:20;">
    ...
    if (el.type === "text") {
      ...
      inner = `<div style="width:100%;height:100%;background:${el.color || " var(--app-accent, #00BCD4)"};border-radius:${br};"></div>`;
...
function buildSlidesHTML(proposal) {
  ...
  <div style="padding:40px 56px;background:#fff;">
    <div style="border-top:2px solid  var(--app-accent, #00BCD4);padding-top:24px;">
